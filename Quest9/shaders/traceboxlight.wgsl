...
fn getLightInfo(lightPos: vec3f, lightDir: vec3f, hitPoint: vec3f, objectNormal: vec3f) -> LightInfo {
  ...
  if (light.params[2] == 1) {
    ...
    out.intensity = light.intensity * max(dot(viewDirection, -objectNormal), 0);
    out.lightdir = lightDir;
  }
  if (light.params[2] == 2) {
    // spotlight
    var viewDirection = normalize(hitPoint - lightPos);
    if(dot(lightDir, viewDirection) > light.params[0]) {
      var intensity = light.intensity;
      var dist = length(hitPoint - lightPos);
      let factor = light.attenuation[0] + dist * light.attenuation[1] + dist * dist * light.attenuation[2];
      intensity *= pow(dot(lightDir, viewDirection), 5);
      intensity /= factor;
      out.intensity = intensity * max(dot(viewDirection, -objectNormal), 0);
      out.lightdir = viewDirection;
    }
...
fn computeOrthogonalMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y) {
    ...
    if (hitInfo.x > 0) { // if there is a hit
      ...
      // hard shadows
      if (light.params[1] == 0 || light.params[1] == 1) {
        ...
        lightInfo = getLightInfo(lightPos, lightDir, hitPt, normal);
        //   7. Cast a secondary ray, which starts at hitPt and go in the direciton -lightDir, and get the hit and/or light information at the next hit location
        var secSpt = hitPt + EPSILON;
        secSpt = transformPt(secSpt);
        var secRdir = -1 * lightDir;
        secRdir = transformDir(secRdir);
        var secHitInfo = rayBoxIntersection(secSpt, secRdir);
        if (secHitInfo.x > 0) {
          var secHitPt = secSpt + secRdir * secHitInfo.x;
          secHitPt = transformHitPoint(secHitPt);
          let secLightInfo = getLightInfo(lightPos, lightDir, secHitPt, normal);
          // 8. modify the light intensity for casting hard shadow
          lightInfo.intensity *= 0.1;
        }
        //   9. finally, modulate the diffuse color by the light
      ...
      if (light.params[1] == 2) {
        ...
        //   Note: I do the light computation in the world coordiantes because the light intensity depends on the distance and angles in the world coordiantes! If you do it in other coordinate system, make sure you transform them properly back to the world one.
        lightInfo = getLightInfo(lightPos, lightDir, hitPt, normal);
        var totalIntensity: vec4f; // or vec3f
        for (var i = 0; i < 5; i++) {
          // Compute a jittered direction by perturbing lightDir a little
          // Note: you will need a random function implemented in WGSL to do so. make sure you use some "seed" to have different randomness for each jitter direction
          var i_f = f32(i);
          var jLightDir = vec3f(lightDir.x + 0.1*random(vec2f(i_f, i_f+1)), lightDir.y + 0.1*random(vec2f(2*i_f, 2*i_f+1)), lightDir.z + 0.1*random(vec2f(3*i_f, 3*i_f+1)));
          var secSpt = hitPt + EPSILON;
          secSpt = transformPt(secSpt);
          var secRdir = -1 * jLightDir;
          secRdir = transformDir(secRdir);
          var secHitInfo = rayBoxIntersection(secSpt, secRdir);
          if (secHitInfo.x > 0) {
            var secHitPt = secSpt + secRdir * secHitInfo.x;
            secHitPt = transformHitPoint(secHitPt);
            var secLightInfo = getLightInfo(lightPos, lightDir, secHitPt, normal);
            // 8. modify the light intensity for casting hard shadow
            secLightInfo.intensity *= 0.1;
            // Sum to the total intensity
            totalIntensity += secLightInfo.intensity;
          }
        }
        totalIntensity /= 5;
        //   9. finally, modulate the diffuse color by the light
        diffuse *= totalIntensity;
      }

      // soft shadows (distance)
      if (light.params[1] == 3) {
        //   4. transform the light to the world coordinates
        //   Note: My light is stationary, so Icancel the camera movement to keep it stationary
        let lightPos = applyMotorToPoint(light.position.xyz, reverse(cameraPose.motor));
        ...
        if (secHitInfo.x > 0) {
          ...
          secHitPt = transformHitPoint(secHitPt);
          var secLightInfo = getLightInfo(lightPos, lightDir, secHitPt, normal);
          // 8. modify the light intensity for distance calculations
          lightInfo.intensity *= max(pow(min(secHitPt.x - hitPt.x, 1.), 0.85), 0.1);
        }
        //   9. finally, modulate the diffuse color by the light
        diffuse *= lightInfo.intensity;
      }
      ...
      if (light.params[3] == 2) {
        ...
        let r = floor(color[0] * 7) / 7;
        let g = floor(color[1] * 7) / 7;
        let b = floor(color[2] * 7) / 7;
        let a = color[3];
        color = vec4f(r, g, b, a);
      }
...
fn random(seed: vec2f) -> f32 {
  return fract(sin(dot(seed.xy, vec2f(12.9898, 78.233))) * 43758.5453123);
}
