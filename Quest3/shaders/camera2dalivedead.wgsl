...
fn vertexMain(@location(0) pos: vec2f, @builtin(instance_index) idx: u32) -> VertexOutput {
  let u = idx % 2048; // we are expecting 256x256, so modulo 256 to get the x index
  let v = idx / 2048; // divide by 256 to get the y index
  let uv = vec2f(f32(u), f32(v)) / 2048; // normalize the coordinates to [0, 1]
  let halfLength = 1.f; // half cell length
  let cellLength = halfLength * 2.f; // full cell length
  let cell = pos / 2048; // divide the input quad into 256x256 pieces
...
fn fragmentMain(@location(0) cellStatus: f32) -> @location(0) vec4f {
  return vec4f(28.f/255, 161.f/255, 82.f/255, 1) * cellStatus; // (R, G, B, A)
}
...
fn computeMain(@builtin(global_invocation_id) cell: vec3u) {
  ...
  let x = cell.x;
  let y = cell.y;
  let neighborsAlive = cellStatusIn[(y) * 2048 + (x + 1)] + cellStatusIn[(y) * 2048 + (x - 1)] +
                       cellStatusIn[(y + 1) * 2048 + (x)] + cellStatusIn[(y - 1) * 2048 + (x)] +
                       cellStatusIn[(y + 1) * 2048 + (x - 1)] + cellStatusIn[(y + 1) * 2048 + (x + 1)] +
                       cellStatusIn[(y - 1) * 2048 + (x - 1)] + cellStatusIn[(y - 1) * 2048 + (x + 1)];
  let i = y * 2048 + x;
  // Compute new status  
  if (cellStatusIn[i] == 1) {
