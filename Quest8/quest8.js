  ...
  window.addEventListener("keydown", (e) => {
    switch (e.key) {
      ...
      case 's': case 'S':   
        ...
        if (curLight == 0) {
          lights[curLight] = new PointLight([1, 1, 1], [0, 0, 0], [1, 0.1, 0.01], curShading);
        }
        else if (curLight == 1) {
          lights[curLight] = new DirectionalLight([1, 1, 1], [Math.sqrt(3), Math.sqrt(3), Math.sqrt(3)], curShading);
        }
        else {
          lights[curLight] = new SpotLight([1, 1, 1], [0, 0, 0], [0, 1, 0], [1, 0.1, 0.01], 0.785, 5, curShading);
        }
        tracerObj.updateLight(lights[curLight]);
        break;
    }
