...
export default class Camera {
  ...
  moveX(d) {
    ...
    let newDir = PGA3D.applyMotorToDir([d, 0, 0], this._pose);
    let dt = PGA3D.createTranslator(newDir[0], newDir[1], newDir[2]);
  ...
  moveY(d) {
    ...
    // Suggest to use PGA3D
    let newDir = PGA3D.applyMotorToDir([0, d, 0], this._pose);
    let dt = PGA3D.createTranslator(newDir[0], newDir[1], newDir[2]);
    let newpose = PGA3D.geometricProduct(dt, this._pose);
  ...
  moveZ(d) {
    ...
    // Suggest to use PGA3D
    let newDir = PGA3D.applyMotorToDir([0, 0, d], this._pose);
    let dt = PGA3D.createTranslator(newDir[0], newDir[1], newDir[2]);
    let newpose = PGA3D.geometricProduct(dt, this._pose);
  ...
  rotateX(d) {
    ...
    // Suggest to use PGA3D
    let newDir = PGA3D.applyMotorToDir([1, 0, 0], this._pose);
    let newSp = PGA3D.applyMotorToDir([0, 0, 0], this._pose);
    let dr = PGA3D.createRotor(d, newDir[0], newDir[1], newDir[2], newSp[0], newSp[1], newSp[2]);
  ...
  rotateY(d) {
    ...
    // Suggest to use PGA3D
    let newDir = PGA3D.applyMotorToDir([0, 1, 0], this._pose);
    let newSp = PGA3D.applyMotorToDir([0, 0, 0], this._pose);
    let dr = PGA3D.createRotor(d, newDir[0], newDir[1], newDir[2], newSp[0], newSp[1], newSp[2]);
    let newpose = PGA3D.geometricProduct(dr, this._pose);
  ...
  rotateZ(d) {
    ...
    // Suggest to use PGA3D
    let newDir = PGA3D.applyMotorToDir([0, 0, 1], this._pose);
    let newSp = PGA3D.applyMotorToDir([0, 0, 0], this._pose);
    let dr = PGA3D.createRotor(d, newDir[0], newDir[1], newDir[2], newSp[0], newSp[1], newSp[2]);
    let newpose = PGA3D.geometricProduct(dr, this._pose);
